...
public class Resolvedor_circuitos {
  ...
  public void resolverCircuito (Circuito circuito) {
    ...
    for (int i = 0; i < mallas.size(); i++) {      
      ...
      for (Ramal ramal : ramales) {
        ramal.modificarCorriente(0);    //Inicializar Corriente en 0
        
        for (int j = 0; j < mallas.size(); j++) {        //Verificar si el ramal hace parte de otra malla
          if (i == j) {
            ramal.modificarCorriente(corrientes[i]);    //Sumar corriente de Malla
            continue;
          }
        
          if (ramalEnCircuito(mallas.get(j), ramal)){    //Si lo es, incluirlo en la ecuación
            println(-corrientes[j]);
            ramal.modificarCorriente(-corrientes[j]);     //Sumar corriente de Malla
          }
        }
      }
    }
  ...
  public double[]generarEcuacionMalla (ArrayList<Circuito> mallas, int pos) {
    ...
    for (int i = 0; i < mallas.size(); i++) {
      println("Revisando Malla " + i);
      for (Ramal ramal : mallas.get(i).obtenerRamales()) {
        println(ramal.obtenerNombre());
      }
    }
    ...
    for (Ramal ramal : ramales) {
      ...
      for (int i = 0; i < mallas.size(); i++) {    //Verificar si el ramal hace parte de otra malla
         if (pos == i)    //No realizar operación en el mismo circuito
           continue;
