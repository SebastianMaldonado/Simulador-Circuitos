...
public class Nodo {
  ...
  //-------------------------|Obtener Vecinos|-------------------------\\
  public ArrayList<Nodo> obtenerVecinos () {
    ArrayList<Nodo> vecinos = new ArrayList<Nodo>();
    
    for (int i = 0; i < this. ramales.size(); i++)
      vecinos.add(this.ramales.get(i).siguienteNodo(this));
    
    return vecinos;
  }
  
 //-------------------------|Obtener Ramal de Vecino|-------------------------\\
  public Ramal obtenerRamalVecino (Nodo nodo) {    
    for (int i = 0; i < this.ramales.size(); i++) {
      Nodo vecino = this.ramales.get(i).siguienteNodo(this);
      
      if (vecino == nodo)
        return this.ramales.get(i);
    }
